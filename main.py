...
async def run_module(module, account_id, key, recipient: Union[str, None] = None, proxy: Optional[str] = None):
    try:
        if recipient:
            await module(account_id, key, TYPE_WALLET, proxy, recipient)
        else:
            await module(account_id, key, TYPE_WALLET, proxy)
...
def _async_run_module(module, account_id, key, recipient, proxy=None):
    asyncio.run(run_module(module, account_id, key, recipient, proxy))
